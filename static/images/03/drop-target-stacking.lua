...
function DropTargetStacking:IsValidDrop(draggable, dropTarget)
	...
	local sourceIndex = UiIndexableImageBus.Event.GetImageIndex(sourceDraggableImage)
	...
	if (UiElementBus.Event.GetNumChildElements(dropTarget) > 0) then
		...
		destIndex = UiIndexableImageBus.Event.GetImageIndex(destDraggableImage)
	end

	if (destType == "" or (sourceType == destType and sourceIndex == destIndex)) then
		return true
	else
		return false
	end
...
function DropTargetStacking:OnDropHoverStart(draggable)
	...
	local isStackable = (UiElementBus.Event.GetNumChildElements(draggableImage)>0)  --Draggable has a counterbox
...
function DropTargetStacking:OnDrop(draggable)
	...
	local draggableImage = UiElementBus.Event.GetChild(draggable, 0)
	local isStackable = (UiElementBus.Event.GetNumChildElements(draggableImage)>0) --added
	if isStackable == true then
